...
​		`--top-ports <number>` 选项，扫描最常见的指定数量个端口。前 n 个端口是由 **/usr/share/nmap/nmap-services**文件确定的。有时候为了全面的目标主机的端口状态，会进行全端口扫描，以免遗漏攻击面。NNS一书第六章，优化Nmap性能：端口选择数据和策略中，给出了一张关于端口扫描数量和有效性的表格，TCP仅需扫描3328个端口就能达到99%的效率，UDP仅需15094个，而达到100%则需要65536全端口扫描。因此是为了保证准确率仅需全端口扫描，还是牺牲1点准确率换取更短的扫描时长，取决于个人选择。
